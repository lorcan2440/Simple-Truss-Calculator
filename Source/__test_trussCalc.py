...
class TrussTests(unittest.TestCase):
    ...
    def test_SDC_truss(self):
        ...
        try:  # Get the results of the truss calculation and display graphic
            my_results = tc.active_truss.Result(
                tc.active_truss, sig_figs=3, solution_method=tc.SolveMethod.NUMPY_STD
            )
            print(my_results)
    ...
    def test_bridge_structure(self):
        ...
        supports = (
            ("A", {"support_type": "pin"}),
            ("E", {"support_type": "roller", "roller_normal": (0, 1)}),
        )

        tc.init_truss("Bridge")
        results = build_from_lists(
            joints,
            bars,
            loads,
            supports,
    ...
    def test_hex_truss(self):
        ...
        supports = [("A", {"support_type": "pin"}), ("D", {"support_type": "pin"})]
    ...
    def __test_with_angled_roller(self):
        ...
        loads = [("C", 1, 2)]
        supports = (
            ("A", {"support_type": "pin", "pin_rotation": 90}),
...
def build_from_lists(
    ...
    bars: tuple[tuple[str, dict]],
    loads: list[tuple[str, float, float]],
    supports: tuple[tuple[str, dict]],
    **res_kwargs
):
