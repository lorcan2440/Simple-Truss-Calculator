...
def build_truss(truss, joints: tuple[tuple], bars: tuple[tuple], loads: tuple[tuple], supports: tuple):
    ...
    for j in supports:
        t.create_support(truss, 'support_' + j.lower(), 'Support ' + j, 'joint_' + j, 'encastre')


def solve_truss(truss):

    try:
        my_results = truss.Result(truss, sig_figs=3)
        print(my_results)
...
def test_case_1():
    ...
    bars = (('AB', medium_2), ('BC', strong), ('CD', medium_1), ('DE', medium_1), ('EF', medium_1), ('AF', medium_2), ('DF', medium_1), ('BF', weak))
    loads = [('C', 0, -0.675)]
    supports = ('A', 'E')
